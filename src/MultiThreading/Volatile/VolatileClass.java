...
public class VolatileClass {
    ...
    2. Similarly, when a volatile variable is read, it is read directly from the main memory and same is true for all
    variables visible to the thread.

    Java compiler reorders instruction to execute lines in parallel. This might causes issues with visibility guarantee
    ...
    write to a volatile variable. Non-volatile variables themselves can re-order but it can not re-order to go after volatile variable.
    ...
    Note: Write to volatile variable or exiting sync block flushes thread visible variables from CPU register to main memory.
     */
